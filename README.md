...
- compatibility flag prerequisites mirror Workers test-runtime requirements:
  ...
  - duplicate compatibility flags are normalized away during runtime option resolution (preserving first-seen order).
...
- CI and preview workflows run typecheck/build quality gates before publishing artifacts; CI additionally enforces the matrix guard for regression coverage drift.
...
- `cloudflare:test` currently supports:
  ...
  - same-worker Durable Object enforcement prefers runtime durable-object binding metadata and excludes bindings configured with `scriptName` (remote worker designators), with fallback to environment namespace discovery if metadata lookup is unavailable
