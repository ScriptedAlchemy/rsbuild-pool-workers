...
- `cloudflare:test` currently supports:
  ...
  - `runInDurableObject` for RPC-callable instance methods (for classes extending `DurableObject`) on stubs from the same worker isolate
    - when a state-like `ctx/state` object is exposed on the stub, callback state receives that object directly (including common `storage` helpers like `get`/`put`/`list`/`delete`/`setAlarm`, transaction helpers, alarm/bookmark APIs, and exposed `id`/`props`/`container`/`waitUntil` helpers when available)
