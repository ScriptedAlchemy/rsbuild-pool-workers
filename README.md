...
- `cloudflare:test` currently supports:
  ...
  - `runInDurableObject` for RPC-callable instance methods (for classes extending `DurableObject`) on stubs from the same worker isolate
    ...
    - TypeScript callback-state typing preserves the concrete state-like subtype exposed on typed `ctx/state` stubs (including typed `props`/custom fields); otherwise it remains `DurableObjectStateLike | DurableObjectStatePlaceholder`
