...
- if both top-level `workers` and `test.poolOptions.workers` are provided, top-level `workers` takes precedence (including async/promise config export forms), and nested function-valued workers options are not evaluated.
