...
describe("rstest CLI integration", () => {
  ...
  test("supports runInDurableObject state props on synthetic stubs", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./do-state-props.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "do-state-props.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runInDurableObject } from "cloudflare:test";

        test("callbacks can read exposed state props", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
          ...
          const props = await runInDurableObject(stub as any, async (_instance, receivedState) => {
            if ("__kind" in receivedState) {
              throw new Error("expected exposed state-like object");
            }
    ...
    await runFixture(files, ({ stdout, stderr }) => {
      expect(stderr).toBe("");
      expect(stdout).toContain('"status": "pass"');
      expect(stdout).toContain("do-state-props.test.ts");
    });
  });
  ...
  test("supports runInDurableObject fallback to stub.state when ctx is non-state-like", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./do-state-fallback.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "do-state-fallback.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runInDurableObject, runDurableObjectAlarm } from "cloudflare:test";

        test("callbacks fall back from invalid ctx to valid state", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
          ...
          class WorkerRpc {
            ...
            ctx: unknown;
            state: unknown;
            constructor(stubId: unknown, badCtx: unknown, stateLike: unknown) {
              this.id = stubId;
              this.ctx = badCtx;
          ...
          const state = {
            storage: {
              async put(key: string, value: number) {
                operations.push("put:" + key + ":" + String(value));
              },
          ...
          const stub = new WorkerRpc(id, { storage: null }, state);
          const value = await runInDurableObject(stub as any, async (_instance, receivedState) => {
            if ("__kind" in receivedState) {
              throw new Error("expected state-like fallback object");
            }
            await receivedState.storage.put?.("count", 42);
    ...
    await runFixture(files, ({ stdout, stderr }) => {
      expect(stderr).toBe("");
      expect(stdout).toContain('"status": "pass"');
      expect(stdout).toContain("do-state-fallback.test.ts");
    });
  });

  test("supports runDurableObjectAlarm fallback to stub.state when ctx lacks getAlarm helper", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./do-alarm-state-fallback.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "do-alarm-state-fallback.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runDurableObjectAlarm } from "cloudflare:test";

        test("alarm helper falls back to state metadata when ctx lacks getAlarm", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
          ...
          expect(operations).toEqual(["getAlarm", "deleteAlarm", "alarm"]);
        });
      `
    };
    ...
    await runFixture(files, ({ stdout, stderr }) => {
      expect(stderr).toBe("");
      expect(stdout).toContain('"status": "pass"');
      expect(stdout).toContain("do-alarm-state-fallback.test.ts");
    });
  });
  ...
  test("supports runInDurableObject fallback to stub.state when ctx storage getter throws", async () => {
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./do-state-fallback-throwing-storage.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "do-state-fallback-throwing-storage.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runInDurableObject, runDurableObjectAlarm } from "cloudflare:test";

        test("callbacks fall back when ctx.storage getter throws", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
          ...
          const badCtx = {};
          Object.defineProperty(badCtx, "storage", {
            configurable: true,
            get() {
          ...
          const stub = new WorkerRpc(id, badCtx, state);
          const value = await runInDurableObject(stub as any, async (_instance, receivedState) => {
            if ("__kind" in receivedState) {
              throw new Error("expected state-like fallback object");
            }
    ...
    await runFixture(files, ({ stdout, stderr }) => {
      expect(stderr).toBe("");
      expect(stdout).toContain('"status": "pass"');
      expect(stdout).toContain("do-state-fallback-throwing-storage.test.ts");
    });
  });
  ...
  test("supports cloudflare:test-internal state props on synthetic stubs", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./internal-do-state-props.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "internal-do-state-props.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runInDurableObject } from "cloudflare:test-internal";

        test("internal alias callbacks can read exposed state props", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
  ...
  test("supports cloudflare:test-internal fallback to stub.state when ctx is non-state-like", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./internal-do-state-fallback.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "internal-do-state-fallback.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runInDurableObject, runDurableObjectAlarm } from "cloudflare:test-internal";

        test("internal alias falls back from invalid ctx to valid state", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
          ...
          const value = await runInDurableObject(stub as any, async (_instance, receivedState) => {
            ...
            await receivedState.storage.put?.("count", 84);
            return receivedState.storage.get?.<number>("count");
          });
          await expect(runDurableObjectAlarm(stub as any)).resolves.toBe(true);
    ...
    await runFixture(files, ({ stdout, stderr }) => {
      expect(stderr).toBe("");
      expect(stdout).toContain('"status": "pass"');
      expect(stdout).toContain("internal-do-state-fallback.test.ts");
    });
  });

  test("supports cloudflare:test-internal runDurableObjectAlarm fallback to stub.state when ctx lacks getAlarm helper", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./internal-do-alarm-state-fallback.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "internal-do-alarm-state-fallback.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runDurableObjectAlarm } from "cloudflare:test-internal";

        test("internal alarm helper falls back to state metadata when ctx lacks getAlarm", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
  ...
  test("supports cloudflare:test-internal fallback to stub.state when ctx storage getter throws", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./internal-do-state-fallback-throwing-storage.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "internal-do-state-fallback-throwing-storage.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env, runInDurableObject, runDurableObjectAlarm } from "cloudflare:test-internal";

        test("internal alias falls back when ctx.storage getter throws", async () => {
          const namespace = env.COUNTER as {
            idFromName(name: string): unknown;
          };
          const id = namespace.idFromName("singleton");
  ...
  test("keeps cloudflare:test-internal env bindings read-only", async () => {
    const packageRoot = process.cwd();
    const configPathImport = path.join(packageRoot, "src", "config", "index.ts").replaceAll("\\", "/");
    ...
    const files: Record<string, string> = {
      "rstest.config.ts": `
        ...
        export default defineWorkersConfig({
          test: {
            include: ["./env-readonly-internal.test.ts"],
            poolOptions: {
              workers: {
                main: "./worker.ts",
                miniflare: {
      ...
      `,
      "env-readonly-internal.test.ts": `
        import { test, expect } from "@rstest/core";
        import { env } from "cloudflare:test-internal";

        test("internal env is immutable", () => {
          expect(env.TOKEN).toBe("abc123");
          expect(() => {
            (env as Record<string, unknown>).TOKEN = "override";
          }).toThrow("Cannot assign to read only property on cloudflare:test env.");
    ...
    await runFixture(files, ({ stdout, stderr }) => {
      expect(stderr).toBe("");
      expect(stdout).toContain('"status": "pass"');
      expect(stdout).toContain("env-readonly-internal.test.ts");
    });
  });
